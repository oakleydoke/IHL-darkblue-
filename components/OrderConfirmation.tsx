...
const OrderConfirmation: React.FC<OrderConfirmationProps> = ({ order: initialOrder, onBackToHome }) => {
  ...
  const handleResync = async () => {
    ...
    try {
      ...
      // Only update if we actually got a valid response
      if (updatedOrder && updatedOrder.status) {
        setOrder(updatedOrder);
      }
    } catch (e) {
      // SILENT CATCH: On an elite platform, we don't show technical timeouts during polling.
  ...
  // Professional Auto-Sync: Poll every 10 seconds if pending
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12 md:py-24 selection:bg-airalo/10 selection:text-airalo">
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-left duration-700">
          <div className="bg-slate-900 rounded-[3rem] p-12 md:p-16 text-white shadow-[0_50px_100px_-20px_rgba(15,23,42,0.3)] relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-16">
              <div className={`w-20 h-20 ${isError ? 'bg-red-500' : isPending ? 'bg-airalo' : 'bg-airalo'} rounded-[2rem] flex items-center justify-center shadow-2xl shadow-airalo/20 relative`}>
                {isPending && !isError && (
                  <div className="absolute inset-0 bg-airalo rounded-[2rem] animate-ping opacity-20"></div>
                )}
              ...
              <div className="text-right">
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500 mb-2">Protocol Status</p>
                <div className="flex items-center gap-2 justify-end">
                   <div className={`w-2 h-2 ${isPending ? 'bg-amber-500' : 'bg-emerald-500'} rounded-full animate-pulse`}></div>
                   <span className="text-[10px] font-black uppercase tracking-widest text-white">
                    {isPending ? 'Sync in Progress' : 'Asset Secured'}
                   </span>
                </div>
              </div>
            </div>
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter uppercase italic mb-8 leading-[0.9]">
              {isError ? 'Node Conflict' : isPending ? 'Securing Asset' : 'Scholar Verified'}
            </h1>
            
            <div className="space-y-8 text-slate-400 font-medium">
              <p className="text-xl leading-relaxed max-w-lg">
                {isError 
                  ? `Infrastructure latency has interrupted the digital provisioning of your asset. Our engineering desk is resolving this manually. Payment is secured.`
                  : isPending
                  ? `Your payment has been cleared by the central node. We are now synchronizing your identity with the global carrier registry.`
              ...
              <div className="grid grid-cols-2 gap-12 pt-12 border-t border-white/5">
                <div>
                  ...
                  <p className="text-white font-mono text-sm tracking-widest font-bold">
                    {order.iccid && order.iccid !== 'PROVISIONING' ? order.iccid : 'AUTHENTICATING...'}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600 mb-3">Order Token</p>
                  <p className="text-white font-mono text-sm tracking-widest font-bold">{order.orderNo || 'SYNC-TOKEN-P'}</p>
                </div>
              </div>
            </div>
            ...
            {isPending && !isError && (
              <div className="mt-12 bg-white/5 border border-white/10 rounded-3xl p-6 flex items-center justify-between group">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-airalo">
                    Contacting carrier node (Attempt {pollCountRef.current + 1})...
                  </p>
                </div>
                <button 
          ...
          <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 flex items-center justify-between">
             <div className="flex items-center gap-6">
               ...
               <div>
                 <p className="font-black text-[10px] uppercase tracking-widest text-slate-400 mb-1">Global Hub</p>
                 <button onClick={onBackToHome} className="font-black text-sm text-slate-900 hover:text-airalo transition-colors">Return to Terminal</button>
               </div>
             </div>
             <div className="text-right hidden md:block">
               <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Scholar Registry</p>
        ...
        <div className="lg:col-span-5 animate-in fade-in slide-in-from-right duration-1000 delay-200">
          <div className="bg-white rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] border border-slate-100 p-8 md:p-12 sticky top-32">
            <div className="flex justify-between items-center mb-12">
               <div>
                 <h4 className="font-black text-slate-900 uppercase text-xs tracking-[0.3em] mb-1">Digital Asset</h4>
            ...
            <div className="aspect-square bg-slate-50 rounded-[3rem] overflow-hidden flex items-center justify-center p-8 mb-12 border border-slate-100 group relative">
              {isError ? (
                <div className="text-center p-10">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Handoff Interrupted</p>
                </div>
              ) : isPending ? (
                <div className="text-center p-10 flex flex-col items-center">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 animate-pulse">Handshaking Node Registry...</p>
                </div>
              ) : view === 'qr' ? (
                qrUrl ? (
                ...
                <div className="w-full space-y-10 p-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Activation Token</p>
                    <div className="bg-white border border-slate-100 p-5 rounded-2xl flex items-center justify-between group/row">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
                ...
                {isPending ? 'Node Synchronization Active' : 'Registry Fully Secure'}
              </div>
              <p className="text-[11px] font-semibold text-slate-400 leading-relaxed bg-slate-50 p-6 rounded-3xl border border-slate-100">
                Installation: Navigate to <span className="text-slate-900 font-black italic uppercase">Settings &gt; Cellular &gt; Add eSIM</span>. Scan the digital asset above while connected to a stable Wi-Fi network.
              </p>
            </div>
          </div>
        </div>

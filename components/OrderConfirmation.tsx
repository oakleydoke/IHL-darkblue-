...
const OrderConfirmation: React.FC<OrderConfirmationProps> = ({ order, onBackToHome }) => {
  const activationCode = order.activationCode && order.activationCode !== 'PROVISIONING_PENDING' && order.activationCode !== 'PROVISIONING_DELAYED'
        ...
        <div className="lg:col-span-3 space-y-8">
          <div className="bg-slate-900 rounded-[3rem] p-12 md:p-16 text-center text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="mt-12 pt-12 border-t border-white/5">
               <div className="flex items-center justify-center gap-4 mb-6">
                 ...
                 <h3 className="text-xl font-black tracking-tight italic">Provisioning Node Active</h3>
               </div>
               <p className="text-slate-400 font-medium leading-relaxed max-w-sm mx-auto">
                 Your digital credentials have been sent to <span className="text-white font-bold">{order.email}</span>. Please scan the QR code to begin installation.
               </p>
               <div className="mt-8 flex items-center justify-center gap-2">
                 <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                 <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">
                   Direct Carrier Uplink Verified
                 </p>
               </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100 flex flex-col md:flex-row items-center gap-10">
             ...
             <div className="flex-1 text-center md:text-left">
                <h4 className="text-2xl font-black text-slate-900 tracking-tight italic">Elite Connectivity Ready</h4>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Single-use activation asset</p>
             </div>
             <button 
          ...
          <div className="bg-white rounded-[3rem] p-10 shadow-2xl border border-slate-100 h-full flex flex-col items-center justify-center relative overflow-hidden">
            ...
            {qrUrl ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-slate-300 text-[9px] mt-4 max-w-[180px]">Carrier verification can take up to 2 minutes.</p>
              </div>
            )}
